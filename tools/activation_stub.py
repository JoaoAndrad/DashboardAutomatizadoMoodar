...
# simple in-memory store for request status
REQUESTS = {}

def _approve_later(request_id: int, delay: float = 5.0):
    import time
    code = str(secrets.randbelow(90000000) + 10000000)  # 8-digit-ish
    time.sleep(delay)
...
async def request_activation(payload: RequestActivation):
    ...
    # schedule approval after short delay to simulate admin action
    import threading
...
async def submit_master_key(payload: SubmitMasterKey):
    # generate a small envelope with 2 synthetic credentials
